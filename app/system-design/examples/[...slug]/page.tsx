...
import { ChapterSidebar, ExamplesSidebar, FocusMode } from "@/components/system-design";
...
export default function ExamplePage({ params }: ExamplePageProps) {
  ...
  const slugAsParams = resolvedParams.slug.join("/");
  const example = getExampleBySlug(slugAsParams);

  if (!example || !example.published) {
    notFound();
  }
  ...
  return (
    <div className="min-h-screen bg-neutral-100 dark:bg-[#080808] text-neutral-900 dark:text-[#fafafa]">
      {mounted && isDesktop && <ExamplesSidebar />}
        ...
        <div className="max-w-5xl mx-auto px-6 lg:px-8 h-12 flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <nav className="ml-3 flex items-center gap-2 text-xs font-mono uppercase tracking-wider">
              ...
              <span className="text-neutral-300 dark:text-[#333] hidden sm:inline">
                /
              </span>
